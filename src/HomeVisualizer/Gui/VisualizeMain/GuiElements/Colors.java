package HomeVisualizer.Gui.VisualizeMain.GuiElements;

import java.awt.Color;

public class Colors {

    public static final Color IN_WORK_STEP_COLOR = new Color(241, 196, 15);
    public static final Color IN_WORK_STEP_COLOR_BORDER = new Color(247, 220, 111);
    public static final Color FINISHED_STEP_COLOR = new Color(82, 190, 128);
    public static final Color FINISHED_STEP_COLOR_BORDER = new Color(125, 206, 160);
    public static final Color CHOOSE_BUTTON_COLOR = new Color(46, 134, 193);
    public static final Color CHOOSE_BUTTON_COLOR_BORDER = new Color(46, 134, 255);
    protected static final Color START_STEPS_COLOR = new Color(46, 134, 193);
    protected static final Color START_STEPS_COLOR_BORDER = new Color(46, 134, 255);
    protected static final Color UNFULFILLED_STEP_COLOR = new Color(238, 75, 43);
    protected static final Color UNFULFILLED_STEP_COLOR_BORDER = new Color(255, 105, 43);
}
