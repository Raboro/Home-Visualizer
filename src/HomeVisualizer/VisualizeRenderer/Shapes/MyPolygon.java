...
public class MyPolygon {
    ...
    private static final double AMBIANT_LIGHT = 0.05;
    ...
    private void updateVisibility() {
        this.isVisible = this.getAverageX() < 0;
    }
    ...
    public void setLighting(MyVector lightVector) {
        ...
        MyVector normal = MyVector.normalize(MyVector.crossProduct(v2, v1));

        double dot = MyVector.dotProduct(normal, lightVector);
        double sign = dot < 0 ? -1 : 1;
        dot = sign * dot * dot;
        dot = (dot + 1) / 2 * 0.8;

        double lightRatio = Math.min(1, Math.max(0, AMBIANT_LIGHT + dot));
